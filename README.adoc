...
- *integration* - IO, contracts, REST tests and all needing spring context in general, runnable from the IDE. Run: `./mvnw verify -pl app`
- *blackBox* - system tests that needs everything to be setup and the application is running from docker. Run: `./mvn verify -pl blackboxtests`
